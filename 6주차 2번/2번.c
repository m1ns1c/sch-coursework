...
int print_way_transfer(SubwayNode* fromHead, SubwayNode* toHead, char* startName, char* endName) {
    ...
    if (!startNode) return 0;
    SubwayNode* endNode = NULL;
    SubwayNode* e = toHead->next;
    while (e != toHead) {
        if (strcmp(e->name, endName) == 0) { endNode = e; break; }
        e = e->next;
    }
    if (!endNode) return 0;
    ...
    while (tr != fromHead) {
        ...
        if (fwd) {
            SubwayNode* tmp = startNode->next;
            while (tmp != fromHead) {
                if (strcmp(tmp->name, tr->name) == 0) {
                    printf(" = %s(환승)", tmp->name);
                    break;
                }
                printf(" = %s", tmp->name);
                tmp = tmp->next;
        ...
        int fwd2 = 0;
        SubwayNode* chk2 = trInTo->next;
        while (chk2 != toHead) {
            if (strcmp(chk2->name, endName) == 0) { fwd2 = 1; break; }
            chk2 = chk2->next;
        }
        if (fwd2) {
...
int main(void) {
    ...
    if (!in) { printf("1호선.txt 파일을 열 수 없습니다.\n"); return 1; }
    while (fscanf(in, "%s", pLine) == 1) dinsert_last(line1, pLine);
    fclose(in);
    in = fopen("2호선.txt", "r");
    if (!in) { printf("2호선.txt 파일을 열 수 없습니다.\n"); return 1; }
    ...
    while (1) {
        ...
        if (cmd == 1) {
            ...
            else printf("이전 역 또는 다음 역을 찾을 수 없습니다.\n");
        } else if (cmd == 2) {
            char delName[100];
            printf("2> 역 삭제\n");
